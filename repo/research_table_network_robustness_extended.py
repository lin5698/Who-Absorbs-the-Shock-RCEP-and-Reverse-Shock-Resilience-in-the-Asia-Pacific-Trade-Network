...
def run_network_robustness_extended():
    ...
    # Pre-RCEP fixed weight matrices (2021Q4)
    fixed_date = pd.Timestamp('2021-12-31')
    
    def get_fixed_w(mode):
        W = build_trade_network_w(df_bilateral, fixed_date, window_quarters=4, mode=mode)
        if isinstance(W, pd.DataFrame):
            return W.reindex(index=RCEP_LIST, columns=RCEP_LIST).fillna(0).values
        return W

    W_fixed_import = get_fixed_w('import')
    W_fixed_export = get_fixed_w('export')
    W_fixed_symmetric = get_fixed_w('symmetric')
    
    configs = [
        ('TV-Import', 'import', 'dynamic'),
        ('TV-Export', 'export', 'dynamic'),
        ('TV-Symmetric', 'symmetric', 'dynamic'),
        ('Fixed-Import', 'import', 'fixed'),
        ('Fixed-Export', 'export', 'fixed'),
        ('Fixed-Symmetric', 'symmetric', 'fixed'),
    ]
    
    summary_results = []
    
    for label, mode, type_w in configs:
        logger.info(f"Processing Network Definition: {label}")
        
        # Build W_list
        W_list = []
        if type_w == 'fixed':
            W_ref = W_fixed_import if mode == 'import' else (W_fixed_export if mode == 'export' else W_fixed_symmetric)
            W_list = [W_ref] * len(dates_y)
        else:
            for d in dates_y:
                W = build_trade_network_w(df_bilateral, d, window_quarters=4, mode=mode)
                if isinstance(W, pd.DataFrame):
    ...
    res_df.to_csv(OUTPUT_DIR / "Table_Alternative_Network_Definitions.csv", index=False)
    print("\n--- Alternative Network Definitions Robustness ---")
    print(res_df.to_string(index=False))

if __name__ == "__main__":
    run_network_robustness_extended()
